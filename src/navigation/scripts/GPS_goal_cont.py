...
class GPS() :
	...
	def __init__(self):
		...
		self.dest_lat= 12.99164445
		self.dest_lon= 80.23119974
		self.init_lat=0.0
		self.init_lon=0.0
		self.current_x=0.0
		self.current_y=0.0
	...
	def goaler(self):
		...
		goal.target_pose.pose.position.x = dist*sin(bearing) + self.current_x
		goal.target_pose.pose.position.y = dist*cos(bearing) + self.current_y
		...
		print "--------------------"
		print "x"
		print dist*sin(bearing) + self.current_x
		print "--------------------"
		print "y"
		print dist*cos(bearing) + self.current_y
